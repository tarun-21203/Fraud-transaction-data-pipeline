...
if len(sys.argv) != 3:
    raise SystemExit(
        "Usage: spark_transform.py <source_csv_filename> <output_parquet_directory>"
    )
...
df_transform = df_transform.withColumn("amount", df_transform["amount"].cast(DoubleType())) \
    ...
    .withColumn("oldbalanceDest", df_transform["oldbalanceDest"].cast(DoubleType())) \
    .withColumn("newbalanceDest", df_transform["newbalanceDest"].cast(DoubleType()))
